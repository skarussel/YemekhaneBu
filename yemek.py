            ...
            try:
                ...
                if fields:
                    ...
                    if (kcal and "gr" in kcal):
                        ...
                        kcal = kcal[:kcal.index("-")] + kcal[kcal.index("("):].strip()
                    if (raw_ings):
                        raw_ings = [x.text for x in raw_ings.find_all("div")]
                        raw_ings = [s for s in raw_ings if re.sub(" *\d+ *gr\.*", "", s)]
                        raw_ings = [s for s in raw_ings if re.sub("\d|\( *Çiğ *\)|\.", "", s)]
                        raw_ings = raw_ings.replace("İçindeki Malzemeler ( Çiğ )\n", "")
                        raw_ings = "\n".join(raw_ings)
                    ...
                    ingredients[meal] = [[kcal,raw_ings],[translated_kcal[0], translated_ings[0]],[translated_kcal[1], translated_ings[1]]]
                    new_ingredients.append(meal)
                        
                        
            except Exception as e:
                print(f"{datetime.now(istanbul)}: Error occured during fetching ingredients")
                print(meal)
                print(kcal)
                print(raw_ings)
                print(e)
