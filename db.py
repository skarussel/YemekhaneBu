...
def connect():
    ...
    client = pymongo.MongoClient(f"mongodb+srv://{username}:{pw}@cluster0.{con_str}.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
    return client 
...
def update_translations(translations):
    ...
    for meal,translation in translations.items():
        db.translations.update_one({"_id": meal}, {"$set":{"translation": translation}}, upsert=True)
